...
namespace Cim.Manager
{
    public class CsvHelper
    {
        ...
        public static bool WriteColumnCsvFile(List<string> headers, List<string> rows, string fileNamePrefix = null)
        {
            try
            {
                var cimConfig = ConfigurationManager.GetSection("cim") as CimConfig;

                var dateTimeFormatOfColumnCsv = cimConfig.CsvFile.DateTimeFormatOfColumnCsv;
                bool writeColumnCsvFile = cimConfig.CsvFile.WriteColumnCsvFile;
                var columnCsvFilePath = cimConfig.CsvFile.ColumnCsvFilePath;
            ...
            List<AddressData> addressDatas, Func<List<string>, List<string>> sortColumns = null)
        {
            ...
            foreach (var messageGroup in messageGroups)
            {
                ...
                foreach (var item in messageGroup)
                {
                    if (results.Columns.Contains(item.VariableId))
                    {
                        dataRow[item.VariableId] = item.Value;
                    }
                }
                results.Rows.Add(dataRow);
            };
